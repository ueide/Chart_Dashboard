function Gallery() {
  ...
  this.addVisual = function(vis) {
    ...
    // Create icon image
    var icon = createImg(vis.iconPath, '');
    icon.addClass('iconSvg');
    icon.parent(menuItem);

    // Create label text
    var label = createSpan(vis.navTitle);
    ...
    menuItem.mouseOver(function(e) {
      ...
      if (li) select('#' + li.id).addClass("hover");
    });
    ...
    menuItem.mouseOut(function(e) {
      ...
      if (li) select('#' + li.id).removeClass("hover");
    });

    menuItem.mouseClicked(function(e) {
      // Always get the parent LI even if clicking on icon or span
      let li = e.target;
      while (li && li.tagName !== 'LI') {
        li = li.parentElement;
      }
      ...
      // Add 'selected' to the clicked menu item
      var el = select('#' + li.id);
      el.addClass('selected');

      self.selectVisual(li.id);
    });
  ...
  this.selectVisualById = function(id) {
  for (let i = 0; i < this.visuals.length; i++) {
    if (this.visuals[i].id === id) {
      this.selectVisual(id);
      return;
    }
  }
  console.warn("No visualisation found with ID:", id);
};
